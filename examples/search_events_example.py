...
try:
    response = api_instance.search_events(
        limit=10,
    ...
    for event in response.events:
        print(f"Event ID: {event.event_id}, Visitor ID: {event.identification.visitor_id if event.identification else '-'}")
    ...
    print("Exception when calling search_events: %s" % e)
    exit(1)
