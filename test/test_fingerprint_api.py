...
class TestFingerprintApi(unittest.TestCase):
    ...
    def test_search_events_all_params(self) -> None:
        ...
        url_params = {
            ...
            'bot': 'good',
            'bot_info': 'all',
            'bot_info_category': [BotInfoCategory.AI_AGENT.value, BotInfoCategory.SECURITY.value],
            'bot_info_identity': [BotInfoIdentity.SPOOFED.value],
            'bot_info_confidence': [BotInfoConfidence.HIGH.value, BotInfoConfidence.MEDIUM.value],
            'bot_info_provider': ['provider'],
            'bot_info_name': ['name1', 'name2'],
            'ip_address': '192.168.0.1/32',
            'asn': 'testAsn',
