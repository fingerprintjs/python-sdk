...
class FingerprintApi:
    ...
    def search_events(
        ...
        bot_info_identity: Annotated[
            Optional[list[BotInfoIdentity]],
            Field(
                description='Filter events by their Bot Info Identity type.  Multiple identity types can be provided using the repeated keys syntax. For example, `bot_info_identity=verified&bot_info_identity=signed`, will match events with a Bot Info Identity of `verified` or `signed`. Other notations like comma-separated or bracket notation are not supported. '
            ),
        ] = None,
        bot_info_confidence: Annotated[
            Optional[list[BotInfoConfidence]],
            Field(
                description='Filter events by their Bot Info Confidence.  Multiple confidences can be provided using the repeated keys syntax. For example, `bot_info_confidence=high&bot_info_confidence=medium`, will match events with a Bot Info Confidence of `high` or `medium`. Other notations like comma-separated or bracket notation are not supported. '
            ),
        ] = None,
        bot_info_provider: Annotated[
    ...
    def _search_events_serialize(
        ...
        if start is not None:
            if isinstance(start, datetime):
                _query_params.append(('start', start.isoformat(timespec='microseconds')))
            elif isinstance(start, date):
                _query_params.append(('start', start.isoformat()))
            else:
                _query_params.append(('start', start))
        # process the query parameters
        if end is not None:
            if isinstance(end, datetime):
                _query_params.append(('end', end.isoformat(timespec='microseconds')))
            elif isinstance(end, date):
                _query_params.append(('end', end.isoformat()))
            else:
